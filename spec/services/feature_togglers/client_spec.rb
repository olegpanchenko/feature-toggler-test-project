...
RSpec.describe FeatureTogglers::Client, type: :model do
  ...
  describe '#enabled_global_settings!' do
    ...
    it 'updates global settings with a enabled status' do
      ...
      expect(client.enabled?(feature_name)).to be(false)
      result = client.enabled_global_settings!(feature_name, extra_data: { custom_data: 'value' })
  ...
  describe '#disabled_global_settings!' do
    let(:feature_name) { 'foobar' }

    it 'creates global settings with a disabled status' do
      expect(client.enabled?(feature_name)).to be(false)
      result = client.disabled_global_settings!(feature_name)
    ...
    it 'updates global settings with a disabled status' do
      ...
      expect(client.enabled?(feature_name)).to be(true)
      result = client.disabled_global_settings!(feature_name, extra_data: { custom_data: 'value' })
  ...
  describe '#whitelisted_client_settings!' do
    ...
    it 'updates client settings with a whitelisted status' do
      ...
      expect(client.enabled?(feature_name)).to be(false)
      result = client.whitelisted_client_settings!(feature_name, extra_data: { custom_data: 'value' })
  ...
  describe '#blacklisted_client_settings!' do
    it 'creates client settings with a blacklisted status' do
      expect(client.enabled?(global_feature_settings.name)).to be(true)
      result = client.blacklisted_client_settings!(global_feature_settings.name)
    ...
    it 'updates client settings with a blacklisted status' do
      ...
      expect(client.enabled?(feature_name)).to be(true)
      result = client.blacklisted_client_settings!(feature_name, extra_data: { custom_data: 'value' })
  ...
  describe '#disabled_by_client_client_settings!' do
    it 'creates client settings with a disabled_by_client status' do
      expect(client.enabled?(global_feature_settings.name)).to be(true)
      result = client.disabled_by_client_client_settings!(global_feature_settings.name)
    ...
    it 'updates client settings with a disabled_by_client status' do
      ...
      expect(client.enabled?(feature_name)).to be(true)
      result = client.disabled_by_client_client_settings!(feature_name, extra_data: { custom_data: 'value' })
